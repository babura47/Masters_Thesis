...
def solve_system2(Ns, params, policy,tol):
    ...
    wait_times2 = linalg.lsqr(M,v, atol =tol, btol=tol)[0]
...
def solve_system1(Ns, params, policy, tol):
    ...
    wait_times1 = linalg.lsqr(M,v, atol = tol, btol=tol)[0]
...
policy= {}
for s in states:
    policy[s] = 0 if (s[0]+1)/params[1] <= (s[1]+1)/params[2] else 1
...
#     s1 = list(s).copy(); s1[0]+=1; s1.append(0)
#     s2 = list(s).copy(); s2[1]+=1; s2.append(0)
#     if abs(wait_times1[tuple(s1)]-wait_times2[tuple(s2)])<=0.01:
#         new_policy[s] = -1
...
#%%
len([k for k,v in policy.items() if v==0])/len(policy)
...
states0 = [s for s in policy_plot.keys() if s[0]==n1 and s[1] == n2]  #here i have set n1=0 and n5=0
policy0 = {s[2:5]:policy_plot[s] for s in states0}
...
colors = ['red' if v == 1 else 'blue' if v==0 else "green" for v in values]
...
ax.set_ylabel('$n_4$')
ax.set_zlabel('$n_5$')
ax.set_title(f'Nash equilibrium strategy when $n_1=${n1}, $n_2=${n2},\narrival rate is {params[0]}, $\mu_3$={params[3]}, other service rates equal to {params[3]}')
...
# Add a legend
red_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='red', markersize=10, label="$S_2$")
blue_patch = plt.Line2D([0], [0], marker='o', color='w', markerfacecolor='blue', markersize=10, label='$S_1$')
